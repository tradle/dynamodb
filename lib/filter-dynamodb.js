...
class FilterOp {
    constructor(opts) {
        ...
        this._normalizeSelect = (select) => {
            const raw = select.concat(values_1.default(this.queriedPrimaryKeys))
                .concat(this.table.primaryKeyProps);
            return uniq_1.default(raw);
        };
        this.guessSelect = () => {
        ...
        this.exec = () => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (checkpoint) {
                ...
                const idx = items.map(this.table.toDBFormat).findIndex(item => {
                    for (let prop in checkpoint) {
                        if (!isEqual_1.default(checkpoint[prop], item[prop])) {
                            return false;
                        }
                    }
                    return true;
            ...
            if (select) {
                ...
                items = items.map(item => pick_1.default(item, select)).map(resource => merge_1.default(table.parseDerivedProps({
                    table,
                    model,
                    resource,
                }), resource));
        ...
        this._maybeInflate = (resource) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            resource = merge_1.default(table.parseDerivedProps({
                table,
                model,
                resource,
            }), resource);
            ...
            if (needsInflate) {
                ...
                if (resource._link && table.objects) {
                    try {
                        more = yield table.objects.get(resource._link);
                    }
                    catch (err) {
                        errors_1.default.rethrow(err, 'developer');
                        this._debug('failed to inflate via object storages', pick_1.default(resource, ['_t', '_link']));
                    }
                }
                else {
                    more = yield table.get(resource);
                }
                if (more) {
        ...
        this.filter = cloneDeep_1.default(this.filter);
...
exports.expandFilter = (table, filter) => {
    const expandedFilter = cloneDeep_1.default(filter);
    if (!filter.EQ)
        return expandedFilter;
    const addProps = (target, noConstants) => extend_1.default(target, utils_1.deriveProps({
        table,
        item: target,
        isRead: true,
        noConstants
