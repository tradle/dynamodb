...
class FilterOp {
    constructor(opts) {
        ...
        this.exec = () => __awaiter(this, void 0, void 0, function* () {
            ...
            let result;
            const { builder, models, orderBy, sortedByDB, filter, after, before, limit, itemToPosition, queryProp, index } = this;
            if (sortedByDB) {
            ...
            if (after || before) {
                ...
                const idx = items.map(this.table.toDBFormat).findIndex(item => {
                    for (let prop in margin) {
                        if (!utils_1.deepEqual(margin[prop], item[prop])) {
                            return false;
                        }
                    }
                    return true;
        ...
        this._addConditions = function () {
            const { prefixedFilter, before, after, opType, builder, sortedByDB, table, index } = this;
        ...
        this._configureBuilder = function _configureBuilder() {
            const { before, after, opType, filter, orderBy, table, queryProp, index, consistentRead, sortedByDB } = this;
            const { EQ } = filter;
            const { type } = EQ;
        ...
        const { table, models, orderBy = constants_2.defaultOrderBy, limit = constants_2.defaultLimit, after, before, consistentRead, forbidScan, bodyInObjects } = this;
        if (before && after) {
