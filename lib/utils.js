...
exports.resolveOrderBy = ({ table, type, hashKey, property, item = {} }) => {
    ...
    if (!(indexedProp && indexedProp.rangeKey))
        return;
    const rangeKeyDerivesFromProp = exports.canRenderTemplate(indexedProp.rangeKey.template, Object.assign({ [property]: 'placeholder' }, item));
    if (rangeKeyDerivesFromProp.full || rangeKeyDerivesFromProp.prefix) {
        return Object.assign({ property: index.rangeKey }, rangeKeyDerivesFromProp);
    }
};
...
exports.deriveProps = ({ table, item, isRead, noConstants }) => {
    ...
    const renderable = _.chain(indexes)
        ...
        .filter(({ template }) => exports.canRenderTemplate(template, item, noConstants).full)
        .value();
