...
exports.defaultIndexedProperties = [
    {
...
const canRenderTemplate = (template, dataKeys) => !_.difference(exports.getTemplateStringVariables(template), dataKeys).length;
const renderTemplate = (str, data) => _.template(str, {
    interpolate: /{{([\s\S]+?)}}/g
})(data);
exports.defaultResolveOrderBy = ({ table, type, hashKey, property }) => {
    const { indexed } = table;
    const index = indexed.find(index => index.hashKey === hashKey);
    const model = table.models[type];
    const { indexedProperties = exports.defaultIndexedProperties } = model;
...
exports.defaultDeriveProperties = ({ table, item, isRead }) => {
