...
exports.getPreferredQueryProperty = ({ type, table, hashKeyProps, filter, orderBy }) => {
    const { indexed } = table;
...
exports.getQueryInfo = ({ table, filter, orderBy, type, index }) => {
    ...
    if (opType === 'query') {
        ...
        if (index) {
            ...
            preferred = exports.getPreferredQueryProperty({
                type,
                table,
...
const findTypeIndexIndex = ({ table, model }) => {
    if (!(table && model))
        debugger;
    return exports.getIndexesForModel({ table, model })
        .findIndex(i => i.hashKey.template.startsWith(`{${constants_1.TYPE}}`));
};
