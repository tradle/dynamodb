...
const transform_1 = tslib_1.__importDefault(require("lodash/transform"));
const identity_1 = tslib_1.__importDefault(require("lodash/identity"));
const zipObject_1 = tslib_1.__importDefault(require("lodash/zipObject"));
const template_1 = tslib_1.__importDefault(require("lodash/template"));
const bindall_1 = tslib_1.__importDefault(require("bindall"));
exports.bindAll = bindall_1.default;
const pify_1 = tslib_1.__importDefault(require("pify"));
exports.promisify = pify_1.default;
...
const schema_joi_1 = tslib_1.__importDefault(require("@tradle/schema-joi"));
...
const { marshall, unmarshall } = aws_sdk_1.default.DynamoDB.Converter;
const fixUnmarshallItem = item => traverse_1.default(item).map(function (value) {
...
exports.levenshteinDistance = (a, b) => fast_levenshtein_1.default.get(a, b);
...
exports.compare = (a, b, propertyName) => {
    const aVal = get_1.default(a, propertyName);
    const bVal = get_1.default(b, propertyName);
    if (aVal < bVal)
        return -1;
    if (aVal > bVal)
        return 1;
...
exports.getUsedProperties = (filter) => {
    const flat = exports.flattenFilter(filter);
    const props = flat.reduce((all, more) => {
        extend_1.default(all, more);
        return all;
    }, {});
...
exports.getModelProperties = memoize_1.default(model => {
...
exports.getQueryInfo = ({ table, filter, orderBy, type }) => {
    ...
    const itemToPosition = function itemToPosition(item) {
        ...
        const indexed = pick_1.default(item, props);
...
exports.getTableDefinitionForModel = ({ models, model }) => {
    ...
    return Object.assign({}, exports.normalizeIndexedProperty(primaryKeys), { tableName: exports.getTableName({ model }), timestamps: false, 
        ...
        schema: schema_joi_1.default({ models, model }), indexes: [], validation: {
            allowUnknown: true
        } });
};
...
exports.toDynogelIndexDefinition = (cloudformation) => {
    ...
    return {
        ...
        projection: pick_1.default(Projection, ['ProjectionType', 'NonKeyAttributes'])
    };
};
...
exports.canRenderTemplate = (template, item, noConstants) => {
    const paths = exports.getTemplateStringVariables(template);
    if (!paths.length && noConstants)
        return false;
    return paths.every(path => typeof get_1.default(item, path) !== 'undefined');
};
const TEMPLATE_SETTINGS = /{([\s\S]+?)}/g;
exports.renderTemplate = (str, data) => {
    const render = template_1.default(str, {
 ...
 */
exports.encodeTemplateValues = data => traverse_1.default(data).map(function (val) {
    if (this.circular)
...
exports.normalizeIndexedProperty = (property) => {
    ...
    return pick_1.default(property, constants_2.PRIMARY_KEYS_PROPS);
};
...
// })
exports.getExpandedProperties = memoize_1.default(({ models, model }) => (Object.assign({}, model.properties, OriginalBaseObjectModel.properties, getNestedProperties({ models, model }))), ({ model }) => model.id);
...
exports.resolveOrderBy = ({ table, type, hashKey, property }) => {
    ...
    if (!(indexedProp && indexedProp.rangeKey))
        return;
    const rangeKeyDerivesFromProp = exports.canRenderTemplate(indexedProp.rangeKey.template, { [property]: 'placeholder' });
...
exports.deriveProps = ({ table, item, isRead, noConstants }) => {
    ...
    const renderable = indexes
        ...
        .filter(({ template }) => exports.canRenderTemplate(template, item, noConstants));
...
exports.parseDerivedProps = ({ table, model, resource }) => {
    const { models } = table;
    const templates = flatMap_1.default(table.getKeyTemplatesForModel(model), ({ hashKey, rangeKey }) => {
        return [
    ...
    const derived = pick_1.default(resource, table.derivedProps);
    const properties = exports.getExpandedProperties({ models, model });
    return transform_1.default(derived, (parsed, value, prop) => {
        ...
        const pathToVal = zipObject_1.default(propPaths, propVals);
        Object.keys(pathToVal).forEach(propPath => {
            ...
            set_1.default(parsed, propPath, val);
        });
    }, {
...
const expandNestedProps = obj => {
    const expanded = {};
    for (let key in obj) {
        set_1.default(expanded, key, obj[key]);
    }
    return expanded;
};
exports.getTableKeys = (def) => {
    ...
    return [hashKey, rangeKey]
        .concat(flatten_1.default(def.indexes.map(def => [def.hashKey, def.rangeKey])))
        .filter(identity_1.default);
};
