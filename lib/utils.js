...
exports.getPreferredQueryProperty = ({ table, hashKeyProps, filter, orderBy }) => {
    const { indexed } = table;
    const { EQ } = filter;
    const type = EQ[constants_1.TYPE];
    const bySpecificity = exports.sortHashKeyPropsBySpecificityDesc({ hashKeyProps, table, type });
    const property = bySpecificity.find(hashKeyProp => {
        ...
        if (rangeKeyIsFilterable)
            return rangeKeyIsFilterable;
        if (!orderBy)
            return false;
...
exports.sortHashKeyPropsBySpecificityDesc = ({ hashKeyProps, table, type }) => {
    if (hashKeyProps.length < 2)
        return hashKeyProps.slice();
    const primaryKeys = Object.assign(Object.assign({}, table.getPrimaryKeysForType(type)), { specificity: Infinity });
...
exports.getQueryInfo = ({ table, filter, orderBy, type }) => {
    ...
    if (opType === 'query') {
        ...
        const preferred = exports.getPreferredQueryProperty({
            table,
        ...
        if (orderBy) {
            ...
            orderBy = Object.assign(Object.assign({}, orderBy), { property: resolvedOrderBy.property });
        }
        else {
    ...
    const itemToPosition = function itemToPosition(item) {
        ...
        return Object.assign(Object.assign({}, indexed), primaryKeyValues);
    };
    return {
        opType,
        hashKey,
...
exports.getTableDefinitionForModel = ({ models, model }) => {
    ...
    return Object.assign(Object.assign({}, exports.normalizeIndexedProperty(primaryKeys)), { tableName: exports.getTableName({ model }), timestamps: false, 
...
exports.getVariablesInTemplate = (str) => {
...
exports.getTemplateStringValues = exports.getVariablesInTemplate;
exports.checkRenderable = (template, item, noConstants) => {
    const vars = exports.getVariablesInTemplate(template);
    const ret = {
        full: false,
        prefix: '',
        vars,
...
exports.getKeyTemplateString = (val) => {
    if (typeof val === 'string') {
        if (exports.getVariablesInTemplate(val).length) {
            return val;
        }
        return `{${val}}`;
    }
...
// })
exports.getExpandedProperties = _.memoize(({ models, model }) => (Object.assign(Object.assign(Object.assign({}, model.properties), OriginalBaseObjectModel.properties), getNestedProperties({ models, model }))), ({ model }) => model.id);
...
exports.resolveOrderBy = ({ table, type, hashKey, property, item = {} }) => {
    ...
    const renderInfo = exports.checkRenderable(indexedProp.rangeKey.template, item);
    let canOrderBy;
    if (renderInfo.full) {
    ...
    return Object.assign({ property: canOrderBy[0] === property ? index.rangeKey : property, canOrderBy }, renderInfo);
};
...
exports.deriveProps = ({ table, item, isRead, noConstants, renderPrefix, }) => {
    ...
    const renderable = _.flatten(nested)
        ...
        const renderable = exports.checkRenderable(template, item, noConstants);
...
exports.parseDerivedProps = ({ table, model, resource }) => {
    ...
    const templates = _.chain(table.getKeyTemplatesForModel(model))
        ...
        return [
            Object.assign(Object.assign({}, hashKey), { type: 'hash' }),
            rangeKey && Object.assign(Object.assign({}, rangeKey), { type: 'range' })
        ];
    })
        .filter(_.identity)
    ...
    return _.transform(derived, (parsed, value, prop) => {
        ...
        const propPaths = exports.getVariablesInTemplate(template);
        const propVals = exports.getVariablesInTemplate(propVal).map(decodeURIComponent);
...
exports.isObjectMinified = (obj) => {
    const cut = obj[constants_2.minifiedFlag];
    return cut && cut.length > 0;
};
exports.createOptimisticLockingCondition = ({ property, value }) => {
    return {
