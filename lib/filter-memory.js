...
const { getRef, isDescendantOf } = validate_resource_1.default.utils;
...
exports.isEqual = ({ models, property, condition, value }) => {
    if (shouldCompareWithDeepEqual({ models, property })) {
        return isEqual_1.default(condition, value);
    }
...
exports.matchesFilter = ({ models, model, object, filter }) => {
    ...
    for (let op in filter) {
        ...
        for (let propertyName in conditions) {
            if (propertyName in operators_1.default) {
            ...
            let isMatch = compare({
                ...
                value: get_1.default(object, propertyName)
            });
            if (!isMatch)
                return false;
        }
...
exports.comparators = {
    ...
    IN: ({ value, condition }) => condition.some(one => isEqual_1.default(one, value)),
    NOT_IN: ({ value, condition }) => condition.every(one => !isEqual_1.default(one, value)),
