...
function isRequired({ model, propertyName }) {
    ...
    if (model.primaryKeys) {
        const schema = utils_1.normalizeIndexedPropertyTemplateSchema(model.primaryKeys);
        return _.flatMap(_.values(schema), ({ template }) => utils_1.getVariablesInTemplate(template))
