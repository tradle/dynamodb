...
const photoIds = resources
    ...
    .slice(0, 20);
utils_1.sortResults({ results: formRequests, orderBy: constants_2.defaultOrderBy });
...
tape_1.default('sortResults', function (t) {
    ...
    const desc = utils_1.sortResults({
        results: formRequests.slice(),
    ...
    // nested
    const ascById = utils_1.sortResults({
    ...
    // fallback to default
    const fallback = utils_1.sortResults({
        results: photoIds.slice()
    });
    const expectedFallback = utils_1.sortResults({
        results: photoIds.slice(),
...
[
    ...
    testNamed('basic pagination', loudAsync((t) => tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        utils_1.sortResults({ results: expected, orderBy });
...
tape_1.default('multiple types, overloaded indexes', loudAsync((t) => tslib_1.__awaiter(this, void 0, void 0, function* () {
    ...
    const table = new _1.Table({
        ...
        deriveProperties: (item, forRead) => {
            let type;
            switch (item[constants_1.TYPE]) {
                case 'tradle.Event':
                    type = item[constants_1.TYPE];
                    let timeAndRand = item[rangeKey];
                    if (!timeAndRand) {
                        if (!forRead) {
                            timeAndRand = item.time + ':' + crypto_1.default.randomBytes(8).toString('hex');
                        }
                    }
                    return {
                        [hashKey]: item[hashKey] || item[constants_1.TYPE] && item.topic && (item[constants_1.TYPE] + ':' + item.topic),
                        [rangeKey]: timeAndRand
                    };
                default:
                    type = 'tradle.Object';
                    return {
                        [hashKey]: item[hashKey] || item[constants_1.TYPE] && item._permalink && `${item[constants_1.TYPE]}_${item._permalink}`,
    ...
    yield table.create();
    yield table.put(event);
    yield table.put(contactInfo);
