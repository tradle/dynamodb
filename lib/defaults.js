...
exports.deriveProps = ({ table, item, isRead }) => {
    ...
    return renderable.reduce((inputs, { property, template, sort }) => {
        const val = utils_1.renderTemplate(template, item);
        if (val.length) {
...
exports.parseDerivedProps = ({ table, model, resource }) => {
    ...
    const properties = utils_1.getExpandedProperties({ models, model });
    return lodash_1.default.transform(derived, (parsed, value, prop) => {
        ...
        const propPaths = utils_1.getTemplateStringVariables(template);
        const propVals = utils_1.getTemplateStringVariables(propVal).map(decodeURIComponent);
        const pathToVal = lodash_1.default.zipObject(propPaths, propVals);
        Object.keys(pathToVal).forEach(propPath => {
            ...
            lodash_1.default.set(parsed, propPath, val);
        });
    }, {
