...
class Table extends events_1.EventEmitter {
    constructor(opts) {
        ...
        this.exportResource = (resource, opts = {}) => {
        ...
        this.reindex = async ({ model, batchSize = 50, findOpts = {} }) => {
            const table = this;
            const { indexes } = model;
            const typeIndex = getIndexForProp({ table, model, property: constants_1.TYPE });
            let checkpoint;
            let updated = 0;
            let unchanged = 0;
...
const getIndexForProp = ({ table, model, property }) => {
    ...
    if (indexIdx === -1) {
        throw new errors_1.default.InvalidInput(`model is not indexed by ${property}`);
    }
