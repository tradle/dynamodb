...
class Table extends events_1.EventEmitter {
    constructor(opts) {
        ...
        const { models, model, objects, exclusive, primaryKeys, requireSigned, forbidScan, readOnly, defaultReadOptions, schema, tableDefinition } = this.opts;
        if (!models)
            throw new Error('expected "models"');
        if (exclusive && !model) {
            throw new Error('expected "model" when "exclusive" is true');
        }
        utils_1.validateTableName(schema.TableName);
        this.schema = schema;
        this.tableDefinition = schema || tableDefinition.TableName
            ? utils_1.toDynogelTableDefinition(schema || tableDefinition)
            : tableDefinition;
