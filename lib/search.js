...
const Errors = tslib_1.__importStar(require("@tradle/errors"));
    ...
    constructor(opts) {
        ...
        this._maybeInflate = async (resource) => {
            ...
            if (needsInflate) {
                ...
                if (more) {
                    return Object.assign(Object.assign({}, resource), more);
                }
            }
            return resource;
        };
        ...
        this._throwIfScanForbidden = function () {
            ...
            const varsNeeded = keys.map(({ hashKey, rangeKey }) => ({
                hashKey: utils_1.getVariablesInTemplate(hashKey.template),
                rangeKey: rangeKey ? utils_1.getVariablesInTemplate(rangeKey.template) : [],
            }));
        ...
        const { table, orderBy, limit = constants_2.defaultLimit, checkpoint, consistentRead, allowScan, bodyInObjects, select } = this;
        const { models } = table;
        this.models = models;
        this.limit = limit;
...
exports.search = (opts) => new Search(opts).exec();
...
exports.expandFilter = (table, filter) => {
    ...
    const addProps = ({ target, noConstants, renderPrefix }) => _.extend(target, table.deriveProps({
        item: target,
        isRead: true,
        noConstants,
        renderPrefix,
